...
export const OFFLINE_DATA: Record<string, {
  explanation: string;
  real_life_example: string;
  quiz: QuizQuestion[];
  keywords: string[];
...
5. **The Structure**: Information flows downwards from parents to children, and each person only reports to one immediate boss (parent node).`,
    quiz: [
      ...
      {
        question: "What is the height of an empty binary tree (no nodes)?",
        options: {
        ...
        correct_answer: "A",
        explanation: "Conventionally, a tree with no nodes has a height of -1, while a tree with only a root node has a height of 0."
      },
      {
...
export function explainTopic(topic: string): string {
  ...
  for (const [key, data] of Object.entries(OFFLINE_DATA)) {
  ...
  return `### 📘 Exploring the Topic: **${topic.replace(/\b\w/g, c => c.toUpperCase())}**

Welcome to your offline learning session! Although we are offline, we can break down **${topic}** into fundamental components.

#### 🗝️ Core Foundations of ${topic.replace(/\b\w/g, c => c.toUpperCase())}
...
export function realLifeExample(topic: string): string {
  const normalized = topic.trim().toLowerCase();
  for (const [key, data] of Object.entries(OFFLINE_DATA)) {
    if (normalized.includes(key) || key.includes(normalized)) {
  ...
  return `### 📖 Real-Life Analogy: The Orchestrated Project

To understand **${topic.replace(/\b\w/g, c => c.toUpperCase())}** without complex jargon, let's look at a relatable real-life scenario:
...
* **The ${topic.replace(/\b\w/g, c => c.toUpperCase())} Solution**:
  - You implement **${topic.replace(/\b\w/g, c => c.toUpperCase())}** rules. You set up labeled bookshelves, split the books into genres (Categories), sort them alphabetically, and create a physical ledger index near the entrance.
  - Now, anyone can walk in, read the signs, and walk directly to their book in less than 30 seconds!
* **The Lesson**: In the same way, **${topic.replace(/\b\w/g, c => c.toUpperCase())}** provides the structure, categorization, and flow-control rules that prevent clutter and make interactions orderly, fast, and scalable in its environment.`;
}
...
export function generateQuiz(topic: string): QuizQuestion[] {
  ...
  return [
    {
      question: `What is the ultimate primary goal of applying ${topic.replace(/\b\w/g, c => c.toUpperCase())} in a system?`,
      options: {
      ...
      correct_answer: "B",
      explanation: `At its core, ${topic.replace(/\b\w/g, c => c.toUpperCase())} is utilized to bring organization, efficiency, and structural integrity to its domain.`
    },
    {
      question: `Which of the following is a fundamental characteristic of ${topic.replace(/\b\w/g, c => c.toUpperCase())}?`,
      options: {
      ...
      correct_answer: "A",
      explanation: `All organized processes like ${topic.replace(/\b\w/g, c => c.toUpperCase())} operate under specific governing guidelines, parameters, or logical states.`
    },
    {
      question: `Why is scaling a challenge when dealing with ${topic.replace(/\b\w/g, c => c.toUpperCase())}?`,
      options: {
    ...
    {
      question: `Which of the following represents the best practice when mastering ${topic.replace(/\b\w/g, c => c.toUpperCase())}?`,
      options: {
    ...
    {
      question: "What is the benefit of using modular, structured patterns?",
      options: {
...
export function evaluateAnswer(topic: string, answer: string): EvaluationResult {
  const normalized = topic.trim().toLowerCase();
  const answerClean = answer.trim().toLowerCase();

  if (answerClean.length < 10) {
    return {
      score: 20,
      feedback: "Your answer is extremely brief.",
      mistakes: "The response lacks detail, descriptions, or core terminology.",
      suggestions: [
        "Try to write at least 2-3 full sentences.",
        "Explain the 'how' and 'why' of the topic."
      ],
  ...
  if (!matched) {
    ...
    targetKeywords = [...words, "system", "process", "rule", "structure", "data", "efficient", "solve"];
    targetGuide = `A valid explanation of ${topic} detailing its core goals, components, and practical applications.`;
  }

  const matchedKeywords = targetKeywords.filter(kw => answerClean.includes(kw));
  const keywordRatio = matchedKeywords.length / Math.max(targetKeywords.length, 1);

  const hasExample = ["example", "analogy", "like", "such as", "imagine", "scenario"].some(ex => answerClean.includes(ex));
  const hasComplexityOrStructure = ["time", "space", "complexity", "structure", "step", "rule", "element", "node"].some(comp => answerClean.includes(comp));

  let score = 30; // Baseline
  score += Math.floor(keywordRatio * 40);
  if (hasExample) score += 15;
  if (hasComplexityOrStructure) score += 15;
  score = Math.min(score, 100);

  const feedback = score >= 70 
    ? "Great effort! You successfully included key concepts in your description." 
    : "You have a basic start, but your explanation could benefit from more precise terminology.";

  const mistakesList: string[] = [];
  if (matchedKeywords.length < 2) {
    mistakesList.push("Missing core technical vocabulary that defines this specific topic.");
  }
  if (!hasExample) {
    mistakesList.push("Your response lacks concrete real-life examples or analogies to ground the theory.");
  }
  if (!hasComplexityOrStructure) {
    mistakesList.push("No discussion on structural mechanics, rules, or performance characteristics (like speed or limits).");
  }

  const mistakes = mistakesList.length > 0 ? mistakesList.join(" | ") : "None! Your structural outline is solid.";

  const suggestions: string[] = [];
  if (matchedKeywords.length < 3) {
    const missingKw = targetKeywords.filter(k => !matchedKeywords.includes(k)).slice(0, 3);
    if (missingKw.length > 0) {
      suggestions.push(`Incorporate important terms such as: ${missingKw.join(", ")}.`);
    }
  }
  if (!hasExample) {
    suggestions.push("Add a simple real-life analogy (e.g., 'This is like...') to demonstrate conceptual clarity.");
  }
  if (!hasComplexityOrStructure) {
    suggestions.push("Discuss how this topic operates step-by-step or mention its limitations/constraints.");
  }
  suggestions.push(`Compare your thoughts with the reference guide: "${targetGuide}"`);
...
export function fullLearningSession(topic: string): LearningSession {
  const explanation = explainTopic(topic);
  const example = realLifeExample(topic);
  const quiz = generateQuiz(topic);

  const summary = `* **Topic Focus**: ${topic.replace(/\b\w/g, c => c.toUpperCase())}
...
* **Efficiency Focus**: Selecting optimal paths to minimize waste and errors.`;
  ...
  const motivational_message = `🌟 **Your AI Tutor says**: 
Learning is about building neural connections. By reading the explanation of **${topic.replace(/\b\w/g, c => c.toUpperCase())}**, mapping it to a real-life analogy, and testing your mind with quizzes, you have stimulated active recall! Keep exploring, stay curious, and remember: every expert was once a beginner who refused to quit! 🚀`;

  return {
    introduction: `### 🎓 Welcome to the Full Learning Session for **${topic.replace(/\b\w/g, c => c.toUpperCase())}**\n\nWe have organized a complete, offline-compatible curriculum to take you from a complete beginner to a confident practitioner. Follow the steps sequentially!`,
    explanation,
